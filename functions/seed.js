// Seed Firestore with an admin + demo data. Run against the emulator:
//   FIRESTORE_EMULATOR_HOST=127.0.0.1:8080 GOOGLE_CLOUD_PROJECT=clarke-mechanical-crm node seed.js
const admin = require('firebase-admin');
const bcrypt = require('bcryptjs');
const { v4: uuid } = require('uuid');
if (!admin.apps.length) admin.initializeApp({ projectId: process.env.GOOGLE_CLOUD_PROJECT || 'clarke-mechanical-crm' });
...
async function set(col, id, data) { await db.collection(col).doc(id).set({ ...data, created_at: data.created_at || now() }); return id; }

(async () => {
  console.log('Seeding Firestore…');

  // Admin
  const adminEmail = 'admin@clarkemechanical.com';
  const existing = await db.collection('users').where('email', '==', adminEmail).limit(1).get();
  if (existing.empty) {
    await set('users', uuid(), { name: 'Clarke Admin', email: adminEmail, password: bcrypt.hashSync('clarke2024', 10), role: 'admin', phone: null });
  }
  ...
  for (const [name, email, role, phone] of techs) {
    ...
    techIds.push(snap.empty ? await set('users', uuid(), { name, email, password: bcrypt.hashSync('clarke2024', 10), role, phone }) : snap.docs[0].id);
  }
  ...
  for (const [name, email, phone, address, city, state, zip] of customers) {
    ...
    custIds.push(snap.empty ? await set('customers', uuid(), { name, email, phone, address, city, state, zip, notes: null }) : snap.docs[0].id);
  }
  ...
  const existingJobs = await db.collection('jobs').limit(1).get();
  ...
  const existingInv = await db.collection('inventory').limit(1).get();
  if (existingInv.empty) {
    for (const [name, sku, category, quantity, min_quantity, unit_price, supplier, location] of inv) {
      await set('inventory', uuid(), { name, sku, description: null, category, quantity, min_quantity, unit_price, supplier, location });
    }
  }

  // Invoices + payments
  const year = today.getFullYear();
  const existingInvoices = await db.collection('invoices').limit(1).get();
  if (existingInvoices.empty) {
    ...
    for (const [status, dayOffset, items] of invoiceDefs) {
      ...
      const id = await set('invoices', uuid(), {
        invoice_number: `INV-${year}-${String(n).padStart(4, '0')}`, customer_id: pick(custIds, n), job_id: null, status,
        issue_date: addDays(dayOffset), due_date: addDays(dayOffset + 30), subtotal, tax_rate: 0.0875, tax_amount, total, notes: null, items: lineItems,
      });
  ...
  // Quotes
  const existingQuotes = await db.collection('quotes').limit(1).get();
  if (existingQuotes.empty) {
    ...
    for (const [status, items] of quoteDefs) {
      ...
      const subtotal = lineItems.reduce((s, i) => s + i.total, 0);
      const tax_amount = +(subtotal * 0.0875).toFixed(2);
      await set('quotes', uuid(), {
        quote_number: `QUO-${year}-${String(n).padStart(4, '0')}`, customer_id: pick(custIds, n), status,
        issue_date: addDays(-5), expiry_date: addDays(25), subtotal, tax_rate: 0.0875, tax_amount, total: +(subtotal + tax_amount).toFixed(2), notes: null, items: lineItems,
      });
    }
  }
