...
// Personal sign-off used on customer-facing documents (invoices, estimates).
// Falls back to the business contact details in Settings.
const OWNER_NAME = process.env.OWNER_NAME || 'Andre D Clarke';
const OWNER_PHONE = process.env.OWNER_PHONE || '+1 (516) 206-6256';
const OWNER_EMAIL = process.env.OWNER_EMAIL || 'Clarkemechanical@gmail.com';
const signature = (b) => `
  <div style="margin-top:26px;padding-top:16px;border-top:1px solid #e2e8f0;">
    <p style="margin:0 0 4px;font-size:15px;color:#334155;">Warm regards,</p>
    <p style="margin:0;font-size:15px;font-weight:700;color:#0f172a;">${OWNER_NAME}</p>
    <p style="margin:0 0 6px;font-size:14px;color:#475569;">${b?.name || 'Clarke Mechanical Inc.'}</p>
    <p style="margin:0;font-size:14px;color:#475569;">
      <a href="tel:${OWNER_PHONE.replace(/[^\d+]/g, '')}" style="color:#1d4ed8;text-decoration:none;">${OWNER_PHONE}</a><br>
      <a href="mailto:${OWNER_EMAIL}" style="color:#1d4ed8;text-decoration:none;">${OWNER_EMAIL}</a>
    </p>
  </div>`;
